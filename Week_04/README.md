...
核心改进原理：写加锁，保证不会发生混乱；写在一个Copy副本上，而不是在原始数据上。（快照的概念，保证读的时候不会被新的改变，有一段时间新老数据同时存在）

以写的时间作为分割线，写之后的读都是读新的，写之前的读都是读旧的

淘宝商品也是有快照的概念，为了避免价格变来变去说不清。

### HashMap

拿空间换时间。 用途：k-V 原理：hash将大变小，进行映射，初始容量16，扩容*2，负载因子0.75.JDK8以后，在链表长度到8 & 数组长度到64时，使用红黑树（坑在于变换到红黑树之后keyset的顺序会变，如果程序依赖于这个顺序就会有错误，所以要么不依赖顺序，要么自己再排序，要么用LinkedHashMap）

### ConcurrentHashMap

Jdk7 初始有16个segment，每个桶里面也是HashMap，分库分表的原理。
